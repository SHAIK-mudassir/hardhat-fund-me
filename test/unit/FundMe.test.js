...
describe("FundMe", () => {
    ...
    describe("fund", () => {
        it("It fails if Enough Eth is not Send", async () => {
    ...
    describe("withdraw", () => {
        ...
        it("Withdraw ETH from a single Founder", async () => {
            ...
            const endingFudMeBalance = await fundMe.provider.getBalance(
                fundMe.address
            )
            const endingdeployerBalance = await fundMe.provider.getBalance(
                deployer
            )
            assert.equal(endingFudMeBalance, "0")
            assert.equal(
                endingdeployerBalance.add(gasCost).toString(),
                startingFundMEBalance.add(startingdeployerBalance).toStirng()
            )
        })
